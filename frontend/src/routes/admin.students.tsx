...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  "Dinh chi":    { label: "Đình chỉ",   color: "bg-destructive/15 text-destructive border-destructive/30" },
  "Đình chỉ":   { label: "Đình chỉ",   color: "bg-destructive/15 text-destructive border-destructive/30" },
  ...
  SUSPENDED:     { label: "Đình chỉ",   color: "bg-destructive/15 text-destructive border-destructive/30" },
    ...
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      <SheetContent side="right" className="w-full sm:max-w-lg overflow-y-auto">
        ...
        <div className="space-y-4">
          <section>
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Thông tin cá nhân</h3>
            <DetailRow
              label="Ngày sinh"
              value={draft.dob}
          ...
          <section>
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Học vụ</h3>
            <DetailRow
              label="Ngành học"
              ...
              editor={(
                ...
                >
                  <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Chọn ngành..." /></SelectTrigger>
                  <SelectContent>
            ...
            <DetailRow
              label="Năm học"
              value={draft.academicYear ? `${draft.academicYear}–${Number(draft.academicYear) + 4}` : "—"}
              editor={<Input className="h-8 text-xs" type="number" value={draft.academicYear ?? ""} onChange={(e) => set("academicYear", e.target.value)} />}
              {...rowActions("academicYear")}
            />
            <DetailRow
              label="Khóa"
              value={draft.cohort}
              editor={<Input className="h-8 text-xs" value={draft.cohort ?? ""} onChange={(e) => set("cohort", e.target.value)} />}
              {...rowActions("cohort")}
            />
            <DetailRow
              label="Lớp hành chính"
              value={student.raw.className ?? "Chưa gán"}
            />
            <DetailRow
              label="Cố vấn học tập"
              value={student.raw.advisorName ?? "—"}
            />
            <DetailRow
              label="Hệ đào tạo"
              value={draft.trainingType}
              editor={<Input className="h-8 text-xs" value={draft.trainingType ?? ""} onChange={(e) => set("trainingType", e.target.value)} />}
          ...
          <section>
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Kết quả học tập</h3>
            <DetailRow label="GPA học kỳ gần nhất" value={student.gpa != null ? student.gpa.toFixed(2) : null} />
            <DetailRow label="CPA tích lũy" value={student.cpa != null ? student.cpa.toFixed(2) : null} />
            <DetailRow label="Tổng tín chỉ tích lũy" value={student.totalCredits != null ? String(student.totalCredits) : null} />
          </section>
        </div>
      </SheetContent>
    </Sheet>
...
function StudentsPage() {
  ...
  const uniqueStatuses = useMemo(() => {
    ...
    for (const r of rows) if (r.status && !seen.has(r.status)) seen.set(r.status, STATUS_LABELS[r.status]?.label ?? r.status);
    ...
    <>
      <div className="flex flex-col gap-1">
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="h-8 w-32 text-xs"><SelectValue placeholder="Tất cả" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Tất cả</SelectItem>
            {uniqueStatuses.map(([raw, label]) => <SelectItem key={raw} value={raw}>{label}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <Select value={majorFilter} onValueChange={setMajorFilter}>
          <SelectTrigger className="h-8 w-40 text-xs"><SelectValue placeholder="Tất cả ngành" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Tất cả ngành</SelectItem>
            {uniqueMajors.map((name) => <SelectItem key={name} value={name}>{name}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <Select value={yearFilter} onValueChange={setYearFilter}>
          <SelectTrigger className="h-8 w-28 text-xs"><SelectValue placeholder="Tất cả" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Tất cả</SelectItem>
      ...
      <div className="flex flex-col gap-1">
        ...
        <Select value={dobYearFilter} onValueChange={setDobYearFilter}>
          <SelectTrigger className="h-8 w-24 text-xs"><SelectValue placeholder="Tất cả" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Tất cả</SelectItem>
            {uniqueDobYears.map((y) => <SelectItem key={y} value={y}>{y}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-1">
          <Input className="h-8 w-20 text-xs" placeholder="0.0" value={gpaMin} onChange={(e) => setGpaMin(e.target.value)} type="number" min={0} max={4} step={0.1} />
          <span className="text-xs text-muted-foreground">–</span>
          <Input className="h-8 w-20 text-xs" placeholder="4.0" value={gpaMax} onChange={(e) => setGpaMax(e.target.value)} type="number" min={0} max={4} step={0.1} />
        </div>
      </div>
    </>
  );
      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            ...
            header: "Địa chỉ",
            render: (s) => (
              <div className="max-w-48 truncate text-xs text-muted-foreground">{s.address || "—"}</div>
            ),
          },
          {
            key: "gpa",
            ...
            render: (s) => (
              <div className="text-xs tabular-nums whitespace-nowrap">
                ...
                <div className="text-muted-foreground">CPA {s.cpa != null ? s.cpa.toFixed(2) : "—"}</div>
                {s.totalCredits != null && (
          ...
          },
          {
            ...
            render: (s) => (
              <div className="flex justify-end gap-1">
                <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setDetailStudent(s)}>
