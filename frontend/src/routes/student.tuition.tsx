...
function TuitionPage() {
  ...
  const payMutation = useMutation({
    ...
    onError: (error) => toast.error(error instanceof Error ? error.message : "Không tạo được link thanh toán"),
  });
  ...
  return (
    <div>
      ...
      {tuitionQuery.isLoading ? (
        <div className="rounded-lg border bg-card p-6 text-sm text-muted-foreground">Đang tải dữ liệu...</div>
      ) : tuition ? (
          ...
          <div className="rounded-xl border bg-card p-5 shadow-sm">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
              <div className="rounded-lg bg-muted/40 p-3">
                ...
                <div className="mt-1 text-lg font-bold tabular-nums">{formatVND(tuition.totalAmount)}</div>
              </div>
              <div className="rounded-lg bg-muted/40 p-3">
                <div className="text-xs text-muted-foreground">Còn lại</div>
                <div className={`mt-1 text-lg font-bold tabular-nums ${!tuition.paid ? "text-destructive" : "text-green-600"}`}>
        ...
        <div className="rounded-lg border bg-card p-6 text-sm text-muted-foreground">Chưa có hóa đơn.</div>
      )}

      {tuitionQuery.isError && (
        <div className="mt-4 text-sm text-destructive">
          {tuitionQuery.error instanceof Error ? tuitionQuery.error.message : "Không tải được học phí"}
        </div>
      )}
    </div>
  );
