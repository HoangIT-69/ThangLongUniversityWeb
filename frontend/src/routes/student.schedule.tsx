...
const periods = [
  ...
  { index: 5, start: "11:00", end: "11:50" },
  { index: 6, start: "13:00", end: "13:50" },
  { index: 7, start: "14:00", end: "14:50" },
  { index: 8, start: "15:00", end: "15:50" },
  { index: 9, start: "16:00", end: "16:50" },
  { index: 10, start: "17:00", end: "17:50" },
  { index: 11, start: "18:00", end: "18:50" },
  { index: 12, start: "19:00", end: "19:50" },
];
...
function SchedulePage() {
  ...
  type CellData = { name: string; room: string; code: string; rowSpan: number; skip: boolean };
  ...
  (scheduleQuery.data ?? []).forEach((item) => {
    ...
    for (let p = item.startPeriod; p <= item.endPeriod; p += 1) {
      cells[`${item.dayOfWeek}-${p}`] = { 
        name: item.courseName, 
        room: item.room ?? "", 
        code: item.classCode,
        rowSpan: p === item.startPeriod ? rowSpan : 1,
        skip: p !== item.startPeriod
      };
    }
  });
        ...
        <table className="w-full min-w-[800px] border-collapse text-sm">
          <thead>
            <tr className="bg-muted/40">
              <th className="w-24 border-b p-2 text-left text-xs uppercase tracking-wide text-muted-foreground">Tiết</th>
          ...
          <tbody>
            {periods.map((p) => (
              <tr key={p.index}>
                <td className="border-b p-2 align-top"><div className="font-semibold">Tiết {p.index}</div><div className="text-[10px] tabular-nums text-muted-foreground">{p.start}-{p.end}</div></td>
                {[1, 2, 3, 4, 5, 6, 7].map((d) => {
                  const cell = cells[`${d}-${p.index}`];
                  if (cell?.skip) return null;

                  return (
                    <td key={d} rowSpan={cell?.rowSpan || 1} className={cn("h-16 border-b border-l p-1.5 align-top", cell && "bg-primary/5")}>
                      {cell && (
                        <div className="flex h-full flex-col justify-center rounded-md border border-primary/30 bg-card p-1.5 text-xs leading-tight shadow-sm">
