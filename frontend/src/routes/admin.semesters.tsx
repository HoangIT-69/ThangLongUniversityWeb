...
function SemestersPage() {
  ...
  const toggleRegistrationMutation = useMutation({
    mutationFn: ({
      ...
      semester: StudentSemesterResponse;
    ...
    onError: (error) =>
      toast.error(error instanceof Error ? error.message : "Không cập nhật được học kỳ"),
  });
  ...
  const lockEnrollmentMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(error instanceof Error ? error.message : "Không chốt được đăng ký học phần"),
  });
  ...
  const lockRetakeMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(error instanceof Error ? error.message : "Không chốt được đăng ký thi lại"),
  });
      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            key: "actions",
            header: "",
            className: "min-w-[420px] text-right",
            searchable: false,
            render: (s) => {
              const toggling =
                toggleRegistrationMutation.isPending &&
                toggleRegistrationMutation.variables?.semester.id === s.id;
              const lockingEnrollment =
                lockEnrollmentMutation.isPending && lockEnrollmentMutation.variables === s.id;
              const lockingRetake =
                lockRetakeMutation.isPending && lockRetakeMutation.variables === s.id;
              return (
