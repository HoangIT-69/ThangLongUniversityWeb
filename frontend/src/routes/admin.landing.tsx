...
function AdminLandingCMS() {
  ...
  const deleteNews = (id: string) =>
    setDraft({ ...draft, news: draft.news.filter((n) => n.id !== id) });
  const addNews = () =>
    setDraft({
      ...draft,
      news: [
  ...
  const updateProgram = (id: string, patch: Partial<LandingProgram>) =>
    setDraft({
      ...draft,
      programs: draft.programs.map((p) => (p.id === id ? { ...p, ...patch } : p)),
    });
  const deleteProgram = (id: string) =>
    setDraft({ ...draft, programs: draft.programs.filter((p) => p.id !== id) });
  const addProgram = () =>
    setDraft({
      ...draft,
      programs: [
        ...draft.programs,
        { id: `p${Date.now()}`, name: "Chương trình mới", description: "", duration: "4 năm" },
      ],
    });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Chỉnh sửa nội dung hiển thị tại trang chủ công khai (/)."
        actions={
          <>
      ...
      <Card className="p-6">
        ...
        <div className="mt-4 grid gap-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label>CTA chính</Label>
              <Input
                value={draft.heroCtaPrimary}
                onChange={(e) => setDraft({ ...draft, heroCtaPrimary: e.target.value })}
              />
            </div>
            <div className="space-y-1.5">
              <Label>CTA phụ</Label>
              <Input
                ...
                onChange={(e) => setDraft({ ...draft, heroCtaSecondary: e.target.value })}
              />
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              ...
              onChange={(e) => setDraft({ ...draft, aboutBody: e.target.value })}
            />
          </div>
        </div>
      </Card>
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" variant="outline" onClick={addProgram} className="gap-1">
        ...
        <div className="mt-4 space-y-3">
          {draft.programs.map((p) => (
            <div
              key={p.id}
              className="grid gap-2 rounded-lg border p-3 sm:grid-cols-[1fr_1fr_120px_auto]"
            >
              <Input
                placeholder="Tên"
                value={p.name}
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" variant="outline" onClick={addNews} className="gap-1">
            <Plus className="h-3 w-3" />
            Thêm
          </Button>
        </div>
        <div className="mt-4 space-y-3">
          {draft.news.map((n) => (
            <div
              key={n.id}
              className="grid gap-2 rounded-lg border p-3 sm:grid-cols-[2fr_140px_auto]"
            >
              <Input
                placeholder="Tiêu đề"
                value={n.title}
              ...
              <Input
                type="date"
                value={n.date}
                onChange={(e) => updateNews(n.id, { date: e.target.value })}
              />
              <Button
                variant="ghost"
                size="icon"
      ...
      <Card className="p-6">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            <Label>Hotline tuyển sinh</Label>
            <Input
              value={draft.admissionsHotline}
              onChange={(e) => setDraft({ ...draft, admissionsHotline: e.target.value })}
            />
          </div>
          <div className="space-y-1.5 sm:col-span-2">
            <Label>Hạn nộp hồ sơ</Label>
            <Input
              value={draft.admissionsDeadline}
              onChange={(e) => setDraft({ ...draft, admissionsDeadline: e.target.value })}
            />
          </div>
        </div>
      </Card>
